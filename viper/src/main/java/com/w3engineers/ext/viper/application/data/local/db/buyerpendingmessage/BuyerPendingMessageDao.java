package com.w3engineers.ext.viper.application.data.local.db.buyerpendingmessage;
...
public interface BuyerPendingMessageDao {
    ...
    @Query("SELECT * FROM buyerpendingmessage WHERE status =:status ORDER BY id ASC LIMIT 1")
    public BuyerPendingMessage getMsgByStatus(int status);

    @Query("SELECT * FROM buyerpendingmessage WHERE status =:status AND ( (is_incomming=1 AND owner =:userAddress) OR ( is_incomming=0 AND sender=:userAddress ))  ORDER BY id ASC LIMIT 1")
